...
const Launch = ({ launch, isLoading }) => {
  ...
  return (
    <div className="justify-content-between">
      <h1 className="nav flex justify-center w-full text-5xl font-bold leading-tight items-center">
        {launch?.mission_name ? (
          <>
            {launch?.mission_name}
            <span className="relative rounded-md shadow-sm ml-8 hidden xl:inline-flex">
              ...
              <span
                className={`relative inline-flex rounded-full h-6 w-6 ${
                  launch?.launch_success ? "bg-green-500" : "bg-yellow-500"
                }`}
              />
            </span>
          </>
      ...
      <div className="flex flex-col items-center my-13 mx-15 text-center sm:text-center">
